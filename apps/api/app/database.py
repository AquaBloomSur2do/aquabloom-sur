...
def get_supabase_client() -> Client | None:
    ...
    try:
        ...
        key = getattr(
            settings, "SUPABASE_SERVICE_KEY", getattr(settings, "supabase_key", None)
        )

        if not url or not key:
            logger.warning(
                "Credenciales de Supabase ausentes o mal nombradas en el entorno."
            )
            return None
...
def check_supabase_connection() -> dict:
    ...
    if not supabase:
        return {
            "status": "error",
            "detail": "Cliente inactivo por falta de credenciales.",
        }
